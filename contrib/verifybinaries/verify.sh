...
###   wescoincore.org and wescoin.org and compares them.
...
WORKINGDIR="/tmp/wescoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://wescoincore.org"
HOST2="https://wescoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="wescoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'wescoin-',
...
if [ $? -ne 0 ]; then
   echo "wescoin.org failed to provide signature file, but wescoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "wescoin.org and wescoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Wescoin Core binary release signing key installed?"
   fi
