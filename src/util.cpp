...
const char * const WESCOIN_CONF_FILENAME = "wescoin.conf";
const char * const WESCOIN_PID_FILENAME = "wescoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "wescoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Wescoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Wescoin
    // Mac: ~/Library/Application Support/Wescoin
    // Unix: ~/.wescoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wescoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Wescoin";
#else
    // Unix
    return pathRet / ".wescoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", WESCOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", WESCOIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Wescoin Core") == std::string::npos) {
